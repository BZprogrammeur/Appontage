...
class Landing(Node):
    ...
    def __init__(self) -> None:
        ...
        self.takeoff_height = -4.6
        self.landing_height = -0.9
        ...
        if self.offboard_setpoint_counter < 10:
            ...
            if self.state == "TAKEOFF":
                ...
                if abs(self.pose[2] - self.takeoff_height) < 0.3:
                    self.get_logger().info("Takeoff successful, switching to TRACKING state")
                    self.state = "TRACKING"

            elif self.state == "TRACKING":
                self.publish_position_setpoint(self.x_target, self.y_target, self.z_target + self.landing_height)

                if abs(self.pose[2] - (self.z_target + self.landing_height)) < 0.2 and abs(self.pose[0] - self.x_target) < 0.1 and abs(self.pose[1] - self.y_target) < 0.1:
                    self.get_logger().info("Tracking successful, switching to LANDING state")
                    self.state = "LANDING"
